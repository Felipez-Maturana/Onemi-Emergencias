...
CREATE OR REPLACE FUNCTION avanceApoyoEconomico(id INT) 
    ...
    BEGIN
      ...
      UPDATE APOYOS_ECONOMICOS SET avance = 100*Cast ( (SELECT RECAUDACION_ACTUAL FROM APOYOS_ECONOMICOS WHERE APOYOS_ECONOMICOS.ID = $1) as decimal(20,3) ) /(SELECT META FROM APOYOS_ECONOMICOS WHERE APOYOS_ECONOMICOS.ID = $1) WHERE APOYOS_ECONOMICOS.ID=$1;
    END;
    $$ LANGUAGE plpgsql;    
...
CREATE OR REPLACE FUNCTION avanceRecoleccion(id INT) 
    RETURNS void AS $$
    BEGIN
      UPDATE RECOLECCIONES SET avance = 100*Cast((SELECT sum(elementos.cantidad) FROM elementos WHERE elementos.id_recoleccion=$1) AS decimal(20,3) ) / (SELECT ELEMENTOSNECESARIOS FROM RECOLECCIONES WHERE RECOLECCIONES.ID = $1) WHERE RECOLECCIONES.ID=$1;
    END;
    $$ LANGUAGE plpgsql;
...
RETURNS void AS $$
    BEGIN
  UPDATE VOLUNTARIADOS SET avance = 100 * Cast(( SELECT COUNT(*) FROM PERSONAS WHERE PERSONAS.id_voluntariado=$1) AS decimal(20,3) ) / (SELECT PERSONAS FROM VOLUNTARIADOS WHERE VOLUNTARIADOS.ID = $1) WHERE VOLUNTARIADOS.ID=$1;
    END;
    $$ LANGUAGE plpgsql;
...
RETURNS void AS $$
    BEGIN
  UPDATE EVENTOS SET avance = 100 * Cast(( SELECT SUM(ACTIVIDADES.RECAUDACION) FROM ACTIVIDADES WHERE ACTIVIDADES.ID_EVENTO=$1) AS decimal(20,3) ) / (SELECT META FROM EVENTOS WHERE EVENTOS.ID = $1) WHERE EVENTOS.ID=$1;
    END;
    $$ LANGUAGE plpgsql;
-- EVENTOS
